...
FILE_ROTATION_SEC = 600       # rotate files every 10 min (override: --rotation-sec)
FFT_QUEUE_MAXLEN = 5000       # ~13 s buffer at ~384 fps
...
# Dequantization range — MUST match FFT_QUANTIZE_DB_MIN/MAX on the Pi 5.
# Written into the HDF5 attrs so each file is self-describing.
DB_MIN_DEFAULT = -120.0
DB_MAX_DEFAULT = 0.0
...
# ── Loss counters ─────────────────────────────────────────────────────────────
# A deque(maxlen=N) silently evicts its oldest item when full, and ZMQ HWM drops
# are silent too.  For POI a silently-lost frame is indistinguishable from a burst
# that was never intercepted — POI would be under-reported with no way to notice.
# Every loss path is therefore counted explicitly so a run can be validated.
fft_zero_frames = 0     # blank frames from the FPGA (counted even when kept)
fft_zero_dropped = 0    # blank frames actually discarded (--drop-zero-frames)
fft_queue_drops = 0     # frames evicted because fft_queue was full
sig_queue_drops = 0
telem_queue_drops = 0
fft_ts_parse_fails = 0  # meta["ts"] unparseable → fell back to HOST clock (breaks POI)
...
def create_fft_file(output_dir: Path) -> tuple:
    ...
    # Per-tune frame counter assigned on the Pi 5: resets to 0 on every retune, then
    # increments 0,1,2,... for each frame kept.  With FFT_STREAM_DECIMATION = 1 the
    # sequence must be CONTIGUOUS within a tune_id, so any gap = a frame lost in the
    # ZMQ/queue path.  This is what turns silent frame loss into a measurable number
    # (and yields the frame-loss-rate metric for free).
    ds_epoch = f.create_dataset(
        "epoch", shape=(0,), maxshape=(None,), chunks=(FFT_CHUNK_N,), dtype=np.uint32
    )
    ...
    f.attrs["db_min"] = DB_MIN_DEFAULT   # dBFS floor   → needed to dequantize uint8
    f.attrs["db_max"] = DB_MAX_DEFAULT   # dBFS ceiling
...
def fft_writer_thread(output_dir: Path):
    ...
    acc_epoch = np.zeros(FFT_CHUNK_N, dtype=np.uint32)
    acc_idx = 0

    # `row` advances in steps of 32, so the old `row % FFT_FLUSH_EVERY == 0` test only
    # fired at multiples of lcm(32, 1000) = 4000.  Count rows since the last flush
    # instead, so the flush interval is honoured exactly.
    rows_since_flush = 0
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            if acc_idx > 0 and (now - last_activity) > 0.2:
                ...
                _flush_fft_chunk(f, ds, acc_fft, acc_ts, acc_center, acc_tune,
                                 acc_epoch, acc_idx, row)
                row += acc_idx
                acc_idx = 0
            time.sleep(0.001)
            continue
        ...
        acc_epoch[acc_idx] = meta.get("epoch", 0)
        acc_idx += 1

        # Full chunk — write direct
        if acc_idx == FFT_CHUNK_N:
            _flush_fft_chunk(f, ds, acc_fft, acc_ts, acc_center, acc_tune,
                             acc_epoch, FFT_CHUNK_N, row)
            row += FFT_CHUNK_N
            rows_since_flush += FFT_CHUNK_N
            acc_idx = 0

        # Periodic flush
        if rows_since_flush >= FFT_FLUSH_EVERY:
            f.flush()
            rows_since_flush = 0
...
def _flush_fft_chunk(f, ds, acc_fft, acc_ts, acc_center, acc_tune, acc_epoch, count, row):
    ...
    ds["epoch"].resize(new_size, axis=0)
    ...
    ds["epoch"][row:new_size] = acc_epoch[:count]
...
def main():
    global FILE_ROTATION_SEC   # must precede any textual use of the name in this scope
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Discard all-zero (blank) FFT frames. NOT recommended for measurement runs: "
             "it punches holes in the contiguous 'epoch' sequence used to detect frame "
             "loss, and destroys the blank-frame record needed to measure blank fraction.",
    )
    parser.add_argument(
        "--no-drop-zero-frames",
        dest="drop_zero_frames",
        action="store_false",
        help="Keep blank frames (DEFAULT — record everything, filter offline).",
    )
    parser.set_defaults(drop_zero_frames=False)
    parser.add_argument(
        "--rotation-sec",
        type=int,
        default=FILE_ROTATION_SEC,
        help=f"Seconds of data per HDF5 file (default {FILE_ROTATION_SEC}). A ~9 min POI "
             f"run sits right on the 600 s boundary and may straddle two files; pass e.g. "
             f"1800 to keep one run in one file.",
    )
    args = parser.parse_args()

    FILE_ROTATION_SEC = int(args.rotation_sec)   # writer threads read this at runtime
    ...
    global fft_zero_frames, fft_zero_dropped, fft_queue_drops
    global sig_queue_drops, telem_queue_drops, fft_ts_parse_fails
    sig_count = 0
    fft_count = 0
    telem_count = 0
    t0 = time.monotonic()
    ...
    try:
        while True:
            ...
            if sig_sock in events:
                ...
                # Enqueue for writer (deque evicts silently when full → count first)
                if len(sig_queue) == SIG_QUEUE_MAXLEN:
                    sig_queue_drops += 1
                sig_queue.append({
            ...
            if telem_sock in events:
                ...
                if len(telem_queue) == TELEM_QUEUE_MAXLEN:
                    telem_queue_drops += 1
                telem_queue.append({
                    "raw_json": raw.decode("utf-8"),
                    "ts_epoch": ts_epoch,
            ...
            if fft_sock in events:
                ...
                # Blank frames (FPGA blanking → all-zero FFT → uint8 0) are always
                # COUNTED; they are only discarded if explicitly asked for.  Keeping
                # them preserves the contiguous `epoch` sequence (so a gap unambiguously
                # means a lost frame) and lets the blank fraction be measured offline.
                is_zero = not fft_uint8.any()
                if is_zero:
                    fft_zero_frames += 1
                ...
                # Timestamp is assigned on the Pi 5 (PTP-synced CLOCK_REALTIME).  The
                # fallback below substitutes the HOST receive time, which includes
                # variable network delay and would corrupt burst↔frame attribution —
                # so it is counted loudly rather than applied silently.
                ts_str = meta.get("ts", "")
                try:
                    ts_epoch = datetime.fromisoformat(ts_str).timestamp()
                except (ValueError, OSError):
                    ts_epoch = time.time()
                    fft_ts_parse_fails += 1
                    if fft_ts_parse_fails <= 5:
                        print(f"[FFT][WARN] bad ts {ts_str!r} → fell back to host clock")

                # deque(maxlen=N).append() silently evicts the oldest item when full,
                # so check for fullness BEFORE appending to make the loss visible.
                if len(fft_queue) == FFT_QUEUE_MAXLEN:
                    fft_queue_drops += 1
                ...
                fft_queue.append((
                    {
                        ...
                        "db_min": meta.get("db_min", DB_MIN_DEFAULT),
                        "db_max": meta.get("db_max", DB_MAX_DEFAULT),
                    },
                    fft_uint8,
                ))
                ...
                if fft_count % 200 == 0:
                    ...
                    print(
                        ...
                        f"zero={fft_zero_frames} qdrop={fft_queue_drops}"
                    )
        ...
        sig_sock.close()
        fft_sock.close()
        telem_sock.close()      # was leaked before
        ctx.term()
        ...
        # Any non-zero eviction count means frames that DID exist were never stored.
        # In a POI run those look exactly like bursts that were never intercepted,
        # biasing POI downward — so the run must be validated before it is trusted.
        print("-" * 72)
        print("[RECV] Run summary")
        print(f"  FFT frames stored     : {fft_count}")
        print(f"  blank (zero) frames   : {fft_zero_frames}"
              f"{'  (DISCARDED)' if args.drop_zero_frames else '  (kept)'}")
        print(f"  fft_queue evictions   : {fft_queue_drops}")
        print(f"  sig_queue evictions   : {sig_queue_drops}")
        print(f"  telem_queue evictions : {telem_queue_drops}")
        print(f"  bad timestamps        : {fft_ts_parse_fails}")
        if fft_queue_drops or fft_ts_parse_fails:
            print("  !! HOST-SIDE DATA LOSS — do not trust POI from this run.")
        else:
            print("  no host-side loss detected")
        print("  (ZMQ HWM drops happen on the Pi 5 and are invisible here — confirm")
        print("   end-to-end integrity by checking for gaps in 'epoch' per tune_id.)")
